...
module.exports = {
  ...
  execute: async (message, args, client, prefix) => {
    ...
    await message.reply({ embeds: [new MessageEmbed().setColor("RANDOM").setDescription(`Searching for **${searchquery}**`)]}).then((msg) =>{
    ...
    if (!song || !song.tracks.length) return message.reply({ content: 'No results were found!' });
      ...
      try {
        ...
        const collector = msg.createMessageComponentCollector({
          filter: (interaction) => {
            if (interaction.user.id === message.author.id) {
              ...
              interaction.reply({ ephemeral: true, content: `Only **${message.author.tag}** can use this help menu, if you want to use it then you have to run the help command again.`}); return false;
            };
          },
          max: 1,
          time : 60000,
        ...
        collector.on("collect", async (interaction) => {
          const playlist = client.emoji.playlist;
