...
module.exports = {
    ...
    run: async (SynthBot, interaction) => {
        ...
        if(interaction.type === InteractionType.ApplicationCommand) {
            const SlashCommands = SynthBot.slashCommands.get(interaction.commandName);
            ...
            if (SlashCommands) {
                ...
                try {
                    if (SlashCommands.owner && SynthBot.config.OwnerId.includes(interaction.user.id)) {
                        interaction.reply({ content: `Im, Not A Fool Bot, Only Owner Can Use This Commands` })
                    }
                    await SlashCommands.run(SynthBot, interaction, prefix);
                } catch (e) {
                    console.log(e);
