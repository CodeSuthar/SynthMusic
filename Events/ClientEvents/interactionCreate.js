      ...
      if (SlashCommands) {
        if (onCoolDown(interaction, SlashCommands)) {
          return interaction.reply({
            embeds: [new MessageEmbed().setColor("RED").setFooter({ text: "You Are On Cooldown" }).setTitle(`❌ Please wait ${onCoolDown(interaction, SlashCommands)} more Second(s) before reusing the \`${interaction.commandName}\` command.`).setTimestamp()],
            ephemeral: true
          });
        };
