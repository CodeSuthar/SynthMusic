    ...
    if (command) {
      if (onCoolDown(message, command)) {
        return message.reply({
          embeds: [new MessageEmbed().setColor("RED").setFooter({ text: "You Are On Cooldown" }).setTitle(`❌ Please wait ${onCoolDown(message, command)} more Second(s) before reusing the \`${command.name}\` command.`).setTimestamp()]
        });
      };
